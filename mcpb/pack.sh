...
# TODO: darwin binary — build on a Mac and drop at target/release/google-mcp
#       then copy to server/google-mcp-darwin and add a darwin platform_override.
...
if [ -f "$WIN_BIN" ]; then
  ...
  echo "WARNING: windows binary not found at $WIN_BIN — bundle will be linux-only"
fi
